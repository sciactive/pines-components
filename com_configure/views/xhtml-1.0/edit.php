...
<form id="p_muid_form" class="pf-form" action="<?php echo htmlentities(pines_url('com_configure', 'save')); ?>" method="post">
	...
	<div class="pf-element pf-full-width">
		<label><span class="pf-label"><input type="checkbox" class="p_muid_default_checkbox ui-widget-content ui-corner-all" name="manset_<?php echo htmlentities($cur_var['name']); ?>" value="ON" <?php echo $is_default ? '' : 'checked="checked" '; ?>/> <?php echo htmlentities($cur_var['cname']); ?></span></label>
		...
		<div class="setting" style="display: none;">
			<?php if (is_array($cur_var['options'])) { ?>
				...
				<div class="pf-group">
					<label><input class="pf-field ui-widget-content ui-corner-all" type="<?php echo is_array($cur_var['value']) ? 'checkbox' : 'radio'; ?>" name="opt_multi_<?php echo htmlentities($cur_var['name']); ?><?php echo is_array($cur_var['value']) ? '[]' : ''; ?>" value="<?php echo addslashes(htmlentities(serialize($cur_option))); ?>" <?php echo ( (is_array($cur_value) && in_array($cur_option, $cur_value) || (!is_array($cur_value) && $cur_value == $cur_option)) ? 'checked="checked" ' : ''); ?>/> <?php echo htmlentities($display); ?></label><br />
				</div>
				<?php } ?>
			<?php } elseif (is_array($cur_var['value'])) { ?>
				<div class="pf-group">
					<?php if (is_int($cur_var['value'][0])) { ?>
					<input class="pf-field ui-widget-content ui-corner-all p_muid_do_tags" type="text" name="opt_int_<?php echo htmlentities($cur_var['name']); ?>" value="<?php echo htmlentities(implode(';;', $cur_value)); ?>" />
					<?php } elseif (is_float($cur_var['value'][0])) { ?>
					<input class="pf-field ui-widget-content ui-corner-all p_muid_do_tags" type="text" name="opt_float_<?php echo htmlentities($cur_var['name']); ?>" value="<?php echo htmlentities(implode(';;', $cur_value)); ?>" />
					<?php } elseif (is_string($cur_var['value'][0])) { ?>
					<div class="pf-field"><textarea rows="3" cols="35" class="ui-widget-content ui-corner-all p_muid_do_tags" style="width: 100%;" name="opt_string_<?php echo htmlentities($cur_var['name']); ?>"><?php echo implode(';;', $cur_value); ?></textarea></div>
					<?php } ?>
				</div>
			<?php } else { ?>
				<?php if (is_bool($cur_var['value'])) { ?>
				<input class="pf-field ui-widget-content ui-corner-all" type="checkbox" name="opt_bool_<?php echo htmlentities($cur_var['name']); ?>" value="ON" <?php echo ($cur_value ? 'checked="checked" ' : ''); ?>/>
				<?php } elseif (is_int($cur_var['value'])) { ?>
				<input class="pf-field ui-widget-content ui-corner-all" type="text" name="opt_int_<?php echo htmlentities($cur_var['name']); ?>" value="<?php echo htmlentities($cur_value); ?>" />
				<?php } elseif (is_float($cur_var['value'])) { ?>
				<input class="pf-field ui-widget-content ui-corner-all" type="text" name="opt_float_<?php echo htmlentities($cur_var['name']); ?>" value="<?php echo htmlentities($cur_value); ?>" />
				<?php } elseif (is_string($cur_var['value'])) { ?>
				<div class="pf-field pf-full-width"><textarea rows="3" cols="35" class="ui-widget-content ui-corner-all" style="width: 100%;" name="opt_string_<?php echo htmlentities($cur_var['name']); ?>"><?php echo $cur_value; ?></textarea></div>
				<?php } else { ?>
				<div class="pf-field pf-full-width"><textarea rows="3" cols="35" class="ui-widget-content ui-corner-all" style="width: 100%;" name="opt_serial_<?php echo htmlentities($cur_var['name']); ?>"><?php echo serialize($cur_value); ?></textarea></div>
				<?php } ?>
			<?php } ?>
		</div>
