	...
	<div class="element full_width">
		<label>
			...
			<?php if (is_array($cur_var['options'])) { ?>
				<div class="group">
					<?php foreach($cur_var['options'] as $cur_option) { ?>
					<label><input class="field ui-widget-content" type="radio" name="opt_multi_<?php echo $cur_var['name']; ?>" value="<?php echo addslashes(htmlentities(serialize($cur_option))); ?>" <?php echo ($cur_var['value'] == $cur_option ? 'checked="checked" ' : ''); ?>/> <?php echo htmlentities((string) $cur_option); ?></label><br />
					<?php } ?>
				</div>
			<?php } else { ?>
