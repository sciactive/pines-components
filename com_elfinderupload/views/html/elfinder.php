...
<script type="text/javascript">
	pines(function(){
		$(".puploader").each(function(){
			var pfile = $(this);
			var unique_id = pfile.data("elfinder_unique_id");
			if (!unique_id) {
			...
			var show_finder = function(){
				...
				pfile.elfdlg = $("<div></div>").appendTo("body");
				...
				pfile.elfdlg.css("overflow", "visible").dialog({
					...
					close: function(){
						pfile.elfdlg.unbind("dialogopen").unbind("select."+pfile.elf.namespace).elfinder("destroy").dialog("destroy").remove();
					}
				});
