...
?>
<form class="pf-form" method="post" id="category_details" action="<?php echo htmlentities(pines_url('com_sales', 'savecategory')); ?>">
		...
		<label>
			...
			<select class="pf-field ui-widget-content" name="parent">
				...
				foreach ($this->categories as $category) {
					...
					<?php
					if ($category->children)
						com_sales_form_category_children($category, $this->entity);
				} ?>
			</select>
		</label>
	</div>
	...
	<div class="pf-element pf-buttons">
		...
		<input class="pf-button ui-state-default ui-priority-secondary ui-corner-all" type="button" onclick="pines.get('<?php echo htmlentities(pines_url('com_sales', 'listcategories')); ?>');" value="Cancel" />
	</div>
</form>