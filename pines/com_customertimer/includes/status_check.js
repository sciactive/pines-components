$(function(){
	var alert_box = $("<div />", {
		"class": "com_customertimer_alert",
		"title": "Customer Timer Warning"
	}).dialog({
		autoOpen: false,
		dialogClass: "com_customertimer_dialog",
		modal: true,
		draggable: false,
		resizable: false,
		buttons: {
			...
			"Hide for 5 Minutes": function(){
				...
				document.cookie = "com_customertimer_stopped=yes; expires="+date.toUTCString()+"; path=/";
	...
	setInterval(function(){
		...
		for(var i=0; i < ca.length; i++) {
			...
			if (c.indexOf("com_customertimer_stopped=") == 0 && c.substring("com_customertimer_stopped=".length,c.length) == "yes")
				return;
		}
		$.ajax({
			url: pines.com_customertimer.status_url,
			type: "GET",
			dataType: "json",
			...
			success: function(data){
				...
				var warnings = [];
				$.each(data, function(){
					if (this.points_remain <= pines.com_customertimer.level_critical)
						criticals = $.merge(criticals, [this]);
					else if (this.points_remain <= pines.com_customertimer.level_warning)
						warnings = $.merge(warnings, [this]);
				});
				if (!criticals.length && !warnings.length)
					return;
				pines.loadcss(pines.rela_location+"components/com_customertimer/includes/status_check.css", false);
				...
				if (criticals.length) {
					...
					$.each(criticals, function(){
						new_criticals.append($("<div />", {
							...
							"css": {"float": "right"},
							"html": (this.points_remain >= 0 ? (this.points_remain * pines.com_customertimer.ppm)+" Minutes Left" : (this.points_remain * -1 * pines.com_customertimer.ppm)+" Minutes Over!")
						})));
					});
