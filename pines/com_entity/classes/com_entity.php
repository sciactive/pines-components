...
class com_entity extends component {
	...
	public function export($filename) {
		...
		if (!$fhandle = fopen($filename, 'w'))
			return false;
		fwrite($fhandle, "# Pines Entity Export\n# Generated by com_entity on ".date('c')."\n\n");
		$offset = 0;
		while ($cur_entity_array = $this->get_entities(array('offset' => $offset, 'limit' => 1))) {
			$offset++;
			$cur_entity = $cur_entity_array[0];
			fwrite($fhandle, "{{$cur_entity->guid}}[".implode(',', $cur_entity->tags)."]\n");
			$data = $cur_entity->get_data();
			foreach ($data as $key => $value) {
				fwrite($fhandle, "\t{$key}=".json_encode(serialize($value))."\n");
			}
		}
		return true;
	}
	...
	public function import($filename) {
		...
		while (!feof($fhandle)) {
			...
			if (preg_match('/^\s*{(\d+)}\[([\w,]+)\]\s*$/', $line, $matches)) {
				...
				if ($guid) {
					$query = sprintf("INSERT INTO `%scom_entity_entities` (`guid`, `tags`, `varlist`) VALUES (%u, '%s', '%s');",
						$pines->config->com_mysql->prefix,
						$guid,
						mysql_real_escape_string(','.implode(',', $tags).',', $pines->db_manager->link),
				...
				$data[$matches[1]] = unserialize(json_decode($matches[2]));
			}
			$line = '';
		}
		return true;
